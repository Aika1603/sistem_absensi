...
?>
		...
		<h3>Jadwal Pelajaran SMAN 5 Karawang
		</h3>
		
			<?php
			if(isset($_GET['aksi']) == 'delete'){ if(isset($_GET['id_pelajaran'])){
			$id_pelajaran =htmlspecialchars(mysqli_real_escape_string($koneksi,$_GET['id_pelajaran']));
			$jam_belajar =htmlspecialchars(mysqli_real_escape_string($koneksi,$_GET['jam_belajar']));
			$hari2 =htmlspecialchars(mysqli_real_escape_string($koneksi,$_GET['hari2']));
...
<div class="col-md-6"><a href="tambahjadwal.php" ><button class=" grid_box1 btn btn-success"> <i class="fa fa-indent nav_icon"></i>Tambah Jadwal</button></a>
<a href="carijadwal.php" ><button class=" grid_box1 btn btn-warning"> <i class="fa fa-search"></i> Search</button></a>
		</div></div>		
		<div class="grid_6 grid_5">
...
//Selecting the data from table but with limit
				...
				if(isset($_POST['cari'])){
					...
					$sql = mysqli_query($koneksi, "SELECT * FROM tb_jadwalpelajaran WHERE hari='$hari' or kelas='$kelas' or materi='$materi' or jam_belajar='$jam_belajar' or id_mengajar='$id_mengajar'");
				}else
				if($hari){
					$sql = mysqli_query($koneksi, "SELECT * FROM tb_jadwalpelajaran WHERE hari='$hari'");
				}else 
				{
					$sql = mysqli_query($koneksi, "SELECT * FROM tb_jadwalpelajaran ");
				};
